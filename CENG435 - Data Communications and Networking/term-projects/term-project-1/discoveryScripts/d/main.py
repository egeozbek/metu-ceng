...
class Server():
    ...
    def startServer(self):
        #here we create worker threads, and make it start
        serverThread = Thread(target = self.serverFunction)
        #here we save the thread reference so that we can call join from main to wait execution to complete
...
if __name__ == "__main__":
    #Server interface addreses
    localAddresses = ['10.10.4.2','10.10.5.2','10.10.7.1']
    #For convenience we have just decided to use consecutive ports
    localPort = 10000
    #we can toggle number of listener threads to
