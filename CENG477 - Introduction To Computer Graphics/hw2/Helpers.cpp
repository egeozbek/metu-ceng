...
/*
 * For rasterization purposes calculate dC color difference between two vertex , with step size
 */

 Color calculateColorStep(Color * firstVertexColorPtr , Color * secondVertexColorPtr, int stepSize){
     return Color((firstVertexColorPtr->r - secondVertexColorPtr->r)/stepSize,
				(firstVertexColorPtr->g - secondVertexColorPtr->g)/stepSize,
				(firstVertexColorPtr->b - secondVertexColorPtr->b)/stepSize);
 }
/*
 * Since we cannot alter color.h/cpp we require some sort of assignment operator
 */
void copyColorValues (Color * targetColor , Color * copySource){
    //cout<<"target color"<<*targetColor<<endl;
    //cout<<"copy source color"<<*copySource<<endl;
    targetColor->r = copySource->r;
    targetColor->g = copySource->g;
    targetColor->b = copySource->b;
}
void addTwoColors (Color * savedColor ,Color * firstColor,Color * secondColor){
    savedColor->r = firstColor->r + secondColor->r;
    savedColor->g = firstColor->g + secondColor->g;
    savedColor->b = firstColor->b + secondColor->b;
}
void multiplyColorWithScalar(Color * targetColor , double multiplier){
    targetColor->r = targetColor->r * multiplier;
    targetColor->g = targetColor->g * multiplier;
    targetColor->b = targetColor->b * multiplier;
}
Vec3 vec4ToVec3(Vec4 & targetVertex){
    return Vec3(targetVertex.x,targetVertex.y,targetVertex.z,-1);
}

void roundVec4ToInteger(Vec4 & targetVertex){
    targetVertex.x =(int) targetVertex.x;
    targetVertex.y =(int) targetVertex.y;
    targetVertex.z =(int) targetVertex.z;
    targetVertex.t =(int) targetVertex.t;
}