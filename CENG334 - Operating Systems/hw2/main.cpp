...
void initializeSemaphores()
{
  ...
  availableCopperInSmelterSemaphore = sem_open("availableCopperInSmelterSemaphore",O_CREAT,0600,0);
  for(unsigned int j = 0 ; j<(unsigned int )copperSmelterIds.size();j++)
  {
    sem_post(availableCopperInSmelterSemaphore);
  }
  sem_unlink("availableIronInSmelterOrFoundrySemaphore");
  for(unsigned int j = 0 ; j<(unsigned int )ironSmelterIds.size();j++)
  {
    sem_post(availableCopperInSmelterSemaphore);
  }
  for(unsigned int j = 0 ; j<(unsigned int)foundryVector.size();j++)
  {
    sem_post(availableCopperInSmelterSemaphore);
  }
  availableIronInSmelterOrFoundrySemaphore = sem_open("availableIronInSmelterOrFoundrySemaphore",O_CREAT,0600,0);
  sem_unlink("availableIronInSmelterOrFoundrySemaphore");
  // cout<<"AvailableCoal in semaphore value is : "<<foundryVector.size()<<endl;
  availableCoalInFoundrySemaphore = sem_open("availableCoalInFoundrySemaphore",O_CREAT,0600,0);
  for(unsigned int j=0;j<(unsigned int)foundryVector.size();j++)
  {
...
void * transporterThreadFunction(void * transporterptr)
{
  ...
  while(true) // TODO here we require a
  {
    ...
    else if(*(transporter->transporterInfo->carry) == IRON)
    {
      ...
      for(unsigned int j = 0; j<ironSmelterIds.size();j++)
      {
        ...
        if(smelterVector[ironSmelterIds[j]]->isAlive && smelterVector[ironSmelterIds[j]]->smelterInfo->waiting_ore_count==1){
          // cout<<"Priority smelter found, id is : "<<ironSmelterIds[j]<<endl;
    ...
    else if(*(transporter->transporterInfo->carry) == COAL)
    {
      ...
      {
        // cout<<"FOUND FOUNDRY AT : "<<foundry->foundryInfo->ID<<endl;
      }
...
void * smelterThreadFunction(void * smelterptr)
{
  ...
  while(true)
  {
    ...
    ts.tv_sec += 5;
    ss =sem_timedwait(smelter->smelterArrivedSemaphore,&ts);
    sss =sem_timedwait(smelter->smelterArrivedSemaphore,&ts);
    if(ss != 0 || sss!=0){
          break;
        }
...
void * foundryThreadFunction(void * foundryptr)
{
  ...
  while(true)
  {
    ...
    clock_gettime(CLOCK_REALTIME, &ts);
    ts.tv_sec += 5;
    ffs =sem_timedwait(foundry->foundryCoalArrivedSemaphore,&ts);
    fffs =sem_timedwait(foundry->foundryIronArrivedSemaphore,&ts); //DOES THIS WORK?
    if(ffs != 0 || fffs!=0){
          break;
        }
    sem_wait(foundry->mutex);
