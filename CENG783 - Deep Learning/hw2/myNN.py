...
def relu(X, ignored):
    ...
    out = np.maximum(X,0)
    dRelu = np.array(X, copy = True)
    dRelu[X > 0] = 1
    dRelu[X <= 0] = 0

    return {'out':out, 'dx':dRelu}
    ...
    for iter in range(num_iters):
        ...
        for i in range(len(network)-2,-1,-1):
            if network[i][0]==fc:
                ...
                dw = np.dot(out_list[i]['dw'],dout)
                db = np.dot(out_list[i]['db'].T,dout)

                network[i][1][0] = network[i][1][0] - eta * dw # <<< YOUR CODE HERE >>> #
                network[i][1][1] = network[i][1][1] - eta * db# <<< YOUR CODE HERE >>> #

                # Propagate gradient by updating dout. dout is the derivative of
                # the loss w.r.t. FC's input. 
                dout = np.dot(dout,out_list[i]['dx'])# <<< YOUR CODE HERE >>> #
