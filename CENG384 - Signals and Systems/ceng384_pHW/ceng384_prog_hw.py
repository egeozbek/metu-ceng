#Partner Name : Bilge Eren ID: 2171585
...
def dft(x):
	...
	for k in range(N):
		current_sum = 0
		power_constant = (-1.j)*k*(2*np.pi/N)
		for n in range (N):
			current_sum+=x[n]*np.power(np.e,power_constant*n)
		F_x[k]=current_sum
	###########################################################################
...
def calculate_spectogram(x, window_size=100, stride=50, dft_function=None):
	...
	S = []
	current_start = 0
	while(N-current_start>window_size):
		temp_vector = dft_function(x[current_start:current_start+window_size])
